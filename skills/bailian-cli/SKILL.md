...
description: >-
  ...
  共享协议（consent / 版本预检 / 鉴权 / 错误上报）在 bailian-protocol；官方安装 `bl skill add --name all`。
...
> Shared protocol → [`../bailian-protocol/SKILL.md`](../bailian-protocol/SKILL.md) (install the full family with `bl skill add --name all`).
> Soft hand-offs by skill name (Read if installed; else `bl … --help` / prompt `bl skill add --name all`): `bailian-gen` (media) · `bailian-finetune` (training) · `bailian-managed-agent` (agents.yaml IaC).
...
>
> **Install (supported):** `bl skill add --name all`
