...
| 合同              | 处理动作                                                                                                                     | 阻塞范围                  |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| 2 GiB 精确字节数  | 用户已确认 2,147,483,648 字节；常量、帮助和独立预期边界已同步                                                                | 已确认                    |
| 媒体模型字段组合  | 管控台与现有资源只读确认 embeddingModelName=qwen3-vl-embedding、rerankModelName=qwen3-vl-rerank；音视频默认 similar 模式     | 已对齐；新建解析未实测    |
...
- 媒体上限经用户确认后修正为 **2 GiB = 2,147,483,648 字节**；边界测试不读取/上传大文件。
- 管控台与现有库只读核对后，媒体建库默认显式发送 embeddingModelName=qwen3-vl-embedding、rerankModelName=qwen3-vl-rerank；未新建/解析媒体进行实测。服务按单库类型与混排范围选择模型的规则见实施报告。
