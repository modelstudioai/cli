...
媒体上限按用户确认改为 **2 GB**。准确字节值仍须与后端合同对齐：是 2,000,000,000 还是 2 × 1024³，不能由测试默默决定。确认后用独立的预期字节值验证规则，避免只读取实现常量再断言实现常量。
...
本次文档交付只完成测试范围与验收方式梳理。待定合同包括 2 GB 精确字节值、媒体默认模型组合、批量导入分批/拒绝策略、chat JSON 新结构及部分通用参数组合；对应测试随合同定稿，不用未经确认的预期锁死实现。
