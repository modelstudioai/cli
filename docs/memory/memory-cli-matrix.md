...
| `--memory-library-id <id>` | string | 除 `node show` / `skill export` 外的全部 | 记忆库 ID,缺省用账号默认库 |
...
| `--memory-library-id` | 32 字符 | 所有带该 flag 的命令 |
...
| 无记忆库管理命令 | 不能建库/改库 | 用默认库,或用 `--memory-library-id` 指向已有库 |
