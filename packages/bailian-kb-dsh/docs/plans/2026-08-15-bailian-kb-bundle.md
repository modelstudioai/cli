...
describe('createKbTools', () => {
  ...
  it('agent_id is required without defaultAgentId and optional with one', () => {
    ...
    expect((withoutDefault.parameters as Record<string, { required?: boolean }>).agent_id!.required).toBe(true)
    expect((withDefault.parameters as Record<string, { required?: boolean }>).agent_id!.required).toBeUndefined()
  })
  ...
  if (err instanceof KbApiError && err.status !== undefined && err.status >= 400 && err.status !== 401 && err.status !== 403) {
    try {
      ...
      if (services.length > 0) {
        throw new KbApiError(`${err.message}. Available services: ${formatServices(services)}`, err.status)
      }
    } catch (hintErr) {
      if (hintErr instanceof KbApiError && hintErr.message.includes('Available services')) throw hintErr
...
export function createKbTools(deps: KbToolDeps) {
  ...
  const search = defineTool({
    ...
    presentCall: args => ({ card: 'generic', title: 'Search knowledge base', kind: 'read', rawInput: args }),
  })
  ...
  const chat = defineTool({
    ...
    presentCall: args => ({ card: 'generic', title: 'Ask knowledge base (may take a few minutes)', kind: 'read', rawInput: args }),
  })
