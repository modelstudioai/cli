...
export interface AuthStore {
  ...
  stored(): { apiKey: boolean; console: boolean };
  ...
  /** 清凭证:console 只删 access_token;all 删 api_key + access_token。返回是否有变更。 */
  logout(scope: "console" | "all"): Promise<boolean>;
}
...
export function makeAuthStore(sources: ResolutionSources): AuthStore {
  return {
    ...
    stored() {
      ...
      return { apiKey: !!file.api_key, console: !!file.access_token };
    },
    ...
    async logout(scope) {
      ...
      const had =
        scope === "console"
          ...
          : existing.access_token !== undefined || existing.api_key !== undefined;
      if (!had) return false;
      delete existing.access_token;
      if (scope === "all") delete existing.api_key;
      await writeConfigFile(existing);
      return true;
    },
  };
